...
export default function Header() {
  ...
  const isFermePage = pathname === "/la-ferme";
  const isTransparentPage = isHomePage || isFermePage;
          ...
          <div className="flex items-center justify-between h-20 lg:h-24">
            ...
            >
              <Image
                ...
                height={60}
                priority
                className="transition-opacity group-hover:opacity-80 h-12 lg:h-14 w-auto"
              ...
              {navigation.map((item) => {
                ...
                return (
                  <Link
                    key={item.href}
                    href={item.href}
                    className={`text-sm font-courier font-medium tracking-wide transition-all hover:text-brand-primary relative group ${
                      isActive
                        ? "text-brand-primary"
                        : isTransparentPage && !isScrolled
                        ? "text-white"
                  ...
                  >
                    {item.label}
                    <span
                      className={`absolute bottom-0 left-0 h-0.5 bg-brand-primary transition-all ${
            ...
            <div className="hidden lg:flex items-center gap-4">
              ...
              <Link
                href="/contact"
                className="bg-brand-primary text-white px-6 py-2.5 rounded-full font-courier font-medium text-sm hover:bg-brand-primary-dark transition-all hover:scale-105 shadow-md"
              >
                Réserver
              </Link>
            </div>
            ...
            <button
              ...
              className={`lg:hidden relative z-10 w-10 h-10 flex flex-col items-center justify-center gap-1.5 transition-colors ${
                isTransparentPage && !isScrolled ? "text-white" : "text-neutral-dark"
              }`}
              aria-label="Menu"
